function widget:GetInfo()
  return {
    ...
    desc      = "Always move unit away from factory's build yard & Remove an accidental build-unit command given to unit in factory.
                 This prevent case of unit stuck in factory & to make sure unit can complete their move queue.",
    author    = "msafwan",
    date      = "2 January 2014",
    license   = "none",
	handler   = false,
...
function widget:UnitFromFactory(unitID, unitDefID, unitTeam, factID, factDefID, userOrders)
	...
	if myTeamID == unitTeam and not excludedFactory[factDefID] then
		...
		else --no command at all? (happens when factory is at edge of map and engine asks unit to rally outside of map)
			local dx,_,dz = Spring.GetUnitDirection(unitID)
			local x,y,z = Spring.GetUnitPosition(unitID)
			dx = dx*100
			dz = dz*100
