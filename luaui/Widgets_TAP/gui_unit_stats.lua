...
local DamageTypeColorTable = {
	bullet = '\255\220\220\220', laser = '\255\255\1\1', plasma = '\255\255\1\255', cannon = '\255\128\128\128',
	thermo = '\255\255\180\1', rocket = '\255\255\255\1', siege = '\255\1\1\255', homing = '\255\1\255\1',
	omni = '\255\1\255\255', explosive = '\255\255\180\180', emp = '\255\180\255\255',
}
...
local lightgreen = '\255\64\180\64'
local green = '\255\1\240\1'
...
local function damageTypeColor(weapDmgType)
	return DamageTypeColorTable[weapDmgType] or white
end
...
-- Colorize the "Damage Multiplier" field according to its range
local function dmgMultColor(oDmg, defaultDmg)
    local mult = oDmg / defaultDmg
    if mult >= 0.9 and mult <= 1.2 then
        return yellow
    elseif mult > 1.2 and mult < 2 then
        return lightgreen
    elseif mult >=2 then
        return green
    elseif mult < 0.9 and mult > 0.5 then
        return orange
    else
        return red
    end
...
function widget:DrawScreen()
	...
	for i = 1, #wepsCompact do
		...
		if uWep.range > 16 then
			...
			-- TODO: Added weapon damage name and type (MaDDoX)
			local weapDmgType = weaponDmgTypes[uDef.name] and weaponDmgTypes[uDef.name][uWep.description] or "Not found"
			local typeName = (weapDmgType ~= "Not found") and uWep.description.." ".. damageTypeColor(weapDmgType)..' ('..weapDmgType..')' or uWep.type
			...
			local defaultDmg = uWep.damages[0]
				...
				if catName and oDmg and (oDmg ~= defaultDmg or cat == 0) then
					...
					if oBurst > 1 then
                        ...
                        if catName == "default" then
                            ...
                            dmgString = format(dmgMultColor(oDmg, defaultDmg) .. "%.2f" .. white .. "x => " .. yellow .. "%.2f \d\p\s", oDmg / defaultDmg, oDmg / oRld)
                        end
					end
